...
public class UInt32LittleEndianSerializer : IPandoSerializer<uint>
{
	...
	public void Serialize(uint value, Span<byte> buffer, INodeDataStore dataStore) => BinaryPrimitives.WriteUInt32LittleEndian(buffer, value);

	public uint Deserialize(ReadOnlySpan<byte> buffer, IReadOnlyNodeDataStore dataStore) => BinaryPrimitives.ReadUInt32LittleEndian(buffer);
}
