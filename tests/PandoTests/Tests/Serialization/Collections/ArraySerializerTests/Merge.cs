...
{
	[Fact]
	public void Should_merge_equal_size_arrays_elementwise()
	{
	...
	[Fact]
	public void Should_merge_into_larger_array_when_target_is_larger()
	{
		var serializer = new ArraySerializer<int>(new Int32LittleEndianSerializer());
		var dataSource = new MemoryDataSource();
	...
	[Fact]
	public void Should_merge_into_larger_array_when_source_is_larger()
	{
		var serializer = new ArraySerializer<int>(new Int32LittleEndianSerializer());
		var dataSource = new MemoryDataSource();
	...
	[Fact]
	public void Should_merge_into_smaller_array_when_base_is_larger()
	{
		var serializer = new ArraySerializer<int>(new Int32LittleEndianSerializer());
		var dataSource = new MemoryDataSource();
	...
	private static void InitializeBuffers(int[] baseArr, int[] targetArr, int[] sourceArr, out Span<byte> baseBuffer, out Span<byte> targetBuffer,
		out Span<byte> sourceBuffer, ArraySerializer<int> serializer, IDataSource dataSource)
	{
		...
		serializer.Serialize(baseArr, baseBuffer, dataSource);
		serializer.Serialize(targetArr, targetBuffer, dataSource);
		serializer.Serialize(sourceArr, sourceBuffer, dataSource);
	}
}
